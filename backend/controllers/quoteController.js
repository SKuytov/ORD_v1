    ...
    try {
        ...
        for (const orderId of order_ids) {
            ...
            if (orders.length > 0) {
                ...
                // Update order: link to quote + supplier, but keep status as-is
                // Status will become 'Quote Requested' only AFTER the RFQ email is actually sent
                await connection.query(
                    `UPDATE orders SET supplier_id = ?, quote_ref = ?
                     WHERE id = ?`,
                    [supplier_id, quoteId, orderId]
                );

                // Log history — RFQ draft created
                await connection.query(
                    `INSERT INTO order_history (order_id, changed_by, field_name, old_value, new_value)
                     VALUES (?, ?, 'quote_ref', NULL, ?)`,
                    [orderId, req.user.name || req.user.username, quoteId]
                );
            }
        }
    ...
    try {
        ...
        for (const order of orders) {
            await connection.query(
                ...
                [order.id, req.user.name || req.user.username]
            );
        }
