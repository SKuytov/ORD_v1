...
router.get('/:documentId/download', authenticateToken, async (req, res) => {
    try {
        ...
        // file_path may be an absolute path (legacy) or just a filename (new)
        ...
        const filePath = document.file_path.startsWith('/')
            ? document.file_path
            : require('path').join(uploadDir, 'documents', document.file_path);
...
router.post('/upload', authenticateToken, upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        if (orderIds.length === 0) {
            ...
            await fs.unlink(req.file.path); // req.file.path is still the full path during upload
            return res.status(400).json({
                success: false,
...
router.delete('/:documentId', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        try {
            const _uploadDir = process.env.UPLOAD_DIR
                ? require('path').resolve(process.env.UPLOAD_DIR)
                : require('path').join(__dirname, '..', 'uploads');
            const _filePath = documents[0].file_path.startsWith('/')
                ? documents[0].file_path
                : require('path').join(_uploadDir, 'documents', documents[0].file_path);
