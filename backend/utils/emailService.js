      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
        <tr>
          ...
          <td align="right" class="hide-sm" style="vertical-align:bottom">
            <p style="margin:0;font-size:10px;color:#334155;letter-spacing:0.5px">
              PARTPULSE.EU
            </p>
          </td>
        </tr>
...
/**
 * Helper: get email(s) of building manager(s) for a given building code.
 * Optionally exclude a specific user ID (to avoid sending duplicate to requester who IS the manager).
 *
 * @param {string} buildingCode  e.g. 'CT', 'WW', 'CBP'
 * @param {number|null} excludeUserId  optional user ID to exclude
 * @returns {Array<{email, name}>}
 */
async function getBuildingManagerEmails(buildingCode, excludeUserId = null) {
    try {
        let query = `
        ...
        console.error('[Email] getBuildingManagerEmails error:', err.message);
        return [];
    }
}
...
/**
 * 8. New Order Notification → Building Manager of that building
 *
 * Triggered when a new order is created in the building manager's building.
 *
 * @param {object} d
 *   orderId, building, itemDescription, quantity, requester, requesterEmail,
 *   dateNeeded, priority, costCenterCode
 */
async function sendBuildingManagerNewOrderNotification(d) {
    const {
        orderId, building, itemDescription, quantity,
        requester, requesterEmail, dateNeeded, priority, costCenterCode,
    } = d;

    const recipients = await getBuildingManagerEmails(building);
...
/**
 * 9. Status Update Notification → Building Manager of that building
 *
 * Triggered whenever any order in the building manager's building changes status.
 *
 * @param {object} d
 *   orderId, building, itemDescription, requesterName, oldStatus, newStatus,
 *   priority, quantity, supplierName?, expectedDelivery?, excludeUserId?
 */
async function sendBuildingManagerStatusUpdateNotification(d) {
    const {
        orderId, building, itemDescription, requesterName,
        oldStatus, newStatus, priority, quantity,
        supplierName, expectedDelivery, excludeUserId,
    } = d;
    ...
    const makeBody = (recipientName) => `
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
             style="background:#f8fafc;border:1px solid ${B.border};border-radius:12px;margin-bottom:24px">
        <tr><td style="padding:22px 24px 20px">
          <p style="margin:0 0 12px;font-size:10px;font-weight:700;letter-spacing:1.5px;
                    text-transform:uppercase;color:${B.muted};font-family:Arial,sans-serif">
            ПРОМЯНА НА СТАТУС
          ...
          <table role="presentation" cellpadding="0" cellspacing="0" border="0">
            <tr>
              <td style="vertical-align:middle">
                <table role="presentation" cellpadding="0" cellspacing="0" border="0"
                       style="background:${oldS.bg || '#f1f5f9'};border:1px solid ${oldS.color}33;border-radius:8px">
                  <tr>
                    <td style="padding:8px 18px;color:${oldS.color};font-size:13px;
                               font-weight:600;font-family:Arial,sans-serif;white-space:nowrap">
                      ${oldS.label}
    ...
    const batchPayload = recipients.map(r => ({
        ...
        tags: [
            { name: 'type',       value: 'bm-status-update' },
            { name: 'order_id',   value: String(orderId) },
            { name: 'new_status', value: newStatus.replace(/\s+/g, '-').toLowerCase() },
        ],
    }));
...
/**
 * Accounting Handover Email
 * Sent to all accounting users when procurement sends a document batch.
 *
 * @param {object} d
 *   handoverId, sentBy, notes, documents[], recipients[], zipPath, zipName
 */
async function sendAccountingHandover(d) {
    const { handoverId, sentBy, notes, documents, recipients, zipPath, zipName } = d;
    if (!recipients || !recipients.length) return { success: false, error: 'No recipients' };

    const docTypeLabel = {
        invoice: 'Фактура', proforma_invoice: 'Проформа фактура',
        delivery_note: 'Доставна бележка', delivery_proof: 'Доказателство за доставка',
        purchase_order: 'Покупателна поръчка', quote_pdf: 'Оферта',
        other: 'Друг документ',
    };
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
        ...
        <thead>
          <tr style="background:#f1f5f9">
            <th style="padding:9px 16px;font-size:11px;color:${B.muted};text-align:left;font-weight:700;letter-spacing:0.8px;text-transform:uppercase">Тип</th>
            <th style="padding:9px 16px;font-size:11px;color:${B.muted};text-align:left;font-weight:700;letter-spacing:0.8px;text-transform:uppercase">Файл</th>
            <th style="padding:9px 16px;font-size:11px;color:${B.muted};text-align:right;font-weight:700;letter-spacing:0.8px;text-transform:uppercase">Размер</th>
          </tr>
        </thead>
        <tbody>${docRows}</tbody>
      </table>

      ${alert('Всички документи са включени в прикачения ZIP архив. Изтеглете го или достъпете системата за преглед.', 'info')}

      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="margin-bottom:16px">
        <tr>
          <td style="padding-right:8px">
            ${btn('✅ Потвърди получаването', `${B.URL}/app.html#accounting/handover/${handoverId}/acknowledge`, '#059669')}
          </td>
          <td style="padding-left:8px">
            ${btn('📂 Отвори в системата', `${B.URL}/app.html#accounting`, B.navyMid)}
          </td>
        </tr>
      </table>
    `;

    // Build zip attachment (only if under 20MB to avoid SMTP issues)
    const attachments = [];
    try {
        const { size } = require('fs').statSync(zipPath);
        if (size < 20 * 1024 * 1024) {
            attachments.push({ path: zipPath, filename: zipName });
        }
    } catch {}
 ...
 * Sent to accounting when an invoice is due soon or overdue.
 *
 * @param {object} d
 *   invoiceMeta { invoice_number, due_date, amount_total, currency, document_id, document_type },
 *   daysUntilDue, recipients[]
 */
async function sendPaymentReminder(d) {
    const { invoiceMeta: inv, daysUntilDue, recipients } = d;
    if (!recipients || !recipients.length) return { success: false, error: 'No recipients' };

    const isOverdue  = daysUntilDue <= 0;
    const urgency    = isOverdue ? 'danger' : daysUntilDue <= 1 ? 'warn' : 'info';
    const daysLabel  = isOverdue
        ? `⚠️ Просрочена с ${Math.abs(daysUntilDue)} дни!`
        : daysUntilDue === 0 ? '🔴 Днес е крайният срок!'
    ...
    const body = `
      ...
      ${alert(
          isOverdue
            ? `<strong>Тази фактура е просрочена!</strong> Моля, извършете плащането незабавно.`
            : `Фактурата трябва да бъде платена до <strong>${fmt(inv.due_date)}</strong>.`,
          urgency
      )}
    ...
    const subject = isOverdue
        ? `🔴 ПРОСРОЧЕНА фактура ${inv.invoice_number || '#' + inv.document_id} — ${amountStr}`
        : `⏰ Напомняне: фактура ${inv.invoice_number || '#' + inv.document_id} — ${daysUntilDue} дни`;

    const batchPayload = recipients.map(r => ({
        to: [r.email],
        subject,
        html: layout({
            preheader: `${daysLabel} — фактура ${inv.invoice_number || ''} ${amountStr}`,
            badge: { label: isOverdue ? '🔴 Просрочена' : '⏰ Напомняне', bg: isOverdue ? '#dc2626' : '#d97706' },
            body,
        }),
    }));
...
module.exports = {
    ...
    sendDailyDigest,
    testEmailConnection,
    // Building Manager notifications
    sendBuildingManagerNewOrderNotification,
    sendBuildingManagerStatusUpdateNotification,
    getBuildingManagerEmails,
    // Accounting Portal
    sendAccountingHandover,
    sendPaymentReminder,
    // Low-level
    sendOne,
    sendBatch,
