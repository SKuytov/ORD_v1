...
function showDashboard() {
    ...
    if (currentUser.role === 'admin') {
        ...
        userRoleBadge.textContent = 'Счетоводство';
    } else {
    ...
    // ⭐ ACCOUNTING: Hide accounting tab by default (shown for eligible roles below)
    const _accTabBtn = document.getElementById('accountingTabButton');
    if (_accTabBtn) _accTabBtn.hidden = true;
    const _mobAccBtn = document.getElementById('mobAccountingBtn');
    if (_mobAccBtn) _mobAccBtn.hidden = true;
    ...
    if (currentUser.role === 'requester') {
        ...
        // ⭐ ACCOUNTING: Accounting users — only see accounting portal + documents
        createOrderSection.classList.add('hidden');
        navTabs.classList.remove('hidden');
        populateStatusFilter();
        // Hide all standard tabs except accounting
        if (orderActionsBar) orderActionsBar.style.display = 'none';
        const orderActionsContainer = document.getElementById('orderActionsContainer');
        if (orderActionsContainer) orderActionsContainer.style.display = 'none';
        // Hide the orders tab from nav (accounting only needs their portal)
        const ordersTabBtn = document.querySelector('.tab[data-tab="ordersTab"]');
        if (ordersTabBtn) ordersTabBtn.style.display = 'none';
    ...
    // ⭐ ACCOUNTING: Show accounting tab button for accounting, admin, procurement, manager roles
    const accountingTabButton = document.getElementById('accountingTabButton');
    if (accountingTabButton) {
        const showAccTab = ['accounting','admin','procurement','manager'].includes(currentUser.role);
        if (showAccTab) accountingTabButton.hidden = false;
    }
    const mobAccountingBtn = document.getElementById('mobAccountingBtn');
    if (mobAccountingBtn) {
        const showAccTab = ['accounting','admin','procurement','manager'].includes(currentUser.role);
        if (showAccTab) mobAccountingBtn.hidden = false;
    }

    // ⭐ ACCOUNTING: Initialize accounting portal (injects section DOM, sets up state)
    if (typeof initAccountingPortal === 'function') initAccountingPortal();

    // For accounting-only users: jump straight to accounting section, skip orders
    if (currentUser.role === 'accounting') {
        document.querySelectorAll('.tab-content').forEach(c => c.classList.add('hidden'));
        currentTab = 'accountingTab';
        if (typeof showAccountingSection === 'function') showAccountingSection();
        return; // Skip loadOrders etc. for accounting role
    }
...
async function openOrderDetail(orderId) {
    try {
        ...
        window._currentDetailOrderId = orderId;
...
function switchTab(tabId) {
    ...
    // ⭐ ACCOUNTING: Route to accounting section (injected, not a .tab-content)
    if (tabId === 'accountingTab') {
        if (typeof showAccountingSection === 'function') showAccountingSection();
        return;
    }

    // When switching away from accounting section, hide it
    const accSection = document.getElementById('accountingSection');
...
async function loadOrderDocuments(orderId) {
    ...
    try {
        ...
        const invoiceTypes = ['invoice','proforma_invoice'];
        const canEditMeta  = currentUser && ['admin','procurement','manager'].includes(currentUser.role);
        const canSendAcc   = currentUser && ['admin','procurement','manager'].includes(currentUser.role);
        ...
        docs.forEach(d => {
            ...
            const size    = formatFileSize(d.file_size || 0);
            const date    = d.uploaded_at ? formatDate(d.uploaded_at) : '';
            const isInv   = invoiceTypes.includes(d.document_type);
            const typeLabel = {
                invoice: '🧾 Фактура',
                proforma_invoice: '📋 Проформа',
                delivery_note: '📦 Дост. бележка',
                purchase_order: '🛒 Пок. поръчка',
                delivery_proof: '📷 Доказателство',
                quote_pdf: '📄 Оферта',
            }[d.document_type] || ('📄 ' + type);

            html += `<div style="display:flex;align-items:center;justify-content:space-between;padding:0.45rem 0.75rem;background:#1e293b;border-radius:6px;border:1px solid #334155;gap:0.5rem;">
                <div style="flex:1;min-width:0;cursor:pointer;" onclick="openDocumentAuthenticated(${d.id})">
                    <span style="color:#e8682a;font-size:0.83rem;font-weight:600;">${typeLabel}</span>
                    <span style="color:#cbd5e1;font-size:0.83rem;margin-left:0.35rem;">${name}</span>
                    <div style="font-size:0.7rem;color:#64748b;margin-top:0.1rem;">${size}${date ? ' · ' + date : ''}</div>
                </div>
                <div style="display:flex;gap:0.35rem;flex-shrink:0;">
                    ${isInv && canEditMeta ? `<button
                        onclick="event.stopPropagation();_accOpenInvoiceModal(${d.id})"
                        style="background:#172334;border:1px solid #2d3e6b;border-radius:6px;
                               color:#94a3b8;font-size:0.72rem;padding:0.25rem 0.55rem;
                               cursor:pointer;white-space:nowrap;"
                        title="Данни за фактура">
                        ✏️ Фактура
                    </button>` : ''}
                    ${d.document_type === 'proforma_invoice' && canSendAcc ? `<button
                        onclick="event.stopPropagation();_accSendProformaModal(${d.id},'${escapeHtml(d.file_name||'').replace(/'/g,"\\'")}')"
                        style="background:#1e3a5f30;border:1px solid #e8682a60;border-radius:6px;
                               color:#e8682a;font-size:0.72rem;padding:0.25rem 0.55rem;
                               cursor:pointer;white-space:nowrap;"
                        title="Изпрати за плащане">
                        📤 Плащане
                    </button>` : ''}
                    <span style="color:#64748b;font-size:0.75rem;cursor:pointer;" onclick="openDocumentAuthenticated(${d.id})">↓</span>
                </div>
            </div>`;
        });
        html += '</div>';
...
// ═══════════════════════════════════════════════════════════════════════════════
// ACCOUNTING PORTAL INTEGRATION BRIDGE
// ═══════════════════════════════════════════════════════════════════════════════
// Bridge functions so the accounting module can work with the existing tab system

function _hideAllSections() {
    // Hide all .tab-content panels (standard tab system)
    document.querySelectorAll('.tab-content').forEach(c => c.classList.add('hidden'));
    // Also hide the injected accounting section if present
    const acc = document.getElementById('accountingSection');
    if (acc) acc.style.display = 'none';
}

function _setActiveNav(btnId) {
    // Deactivate all nav tabs
    document.querySelectorAll('.tab').forEach(t => t.classList.remove('active'));
    // Activate the accounting nav button
...
// ═══════════════════════════════════════════════════════════════════════════════
// ACCOUNTING PORTAL — Frontend module for app.js
// ═══════════════════════════════════════════════════════════════════════════════
// HOW TO INTEGRATE:
//   1. Paste this entire file at the end of app.js (before the last closing lines)
//   2. In the navigation section of app.js, add the accounting tab for the
//      'accounting' role and call initAccountingPortal() on login.
//   3. In the role-switch / navigation init block, add:
//        if (currentUser.role === 'accounting') showAccountingDashboard();
// ═══════════════════════════════════════════════════════════════════════════════

// ── State ─────────────────────────────────────────────────────────────────────
let _accCurrentTab    = 'dashboard';
let _accInvoiceFilter = 'all';
let _accPage          = 1;

// ── Bootstrap ─────────────────────────────────────────────────────────────────
function initAccountingPortal() {
    const role = currentUser?.role;
    // Accounting tab visible to accounting + admin + procurement + manager
    const canSeeAccounting = ['accounting','admin','procurement','manager'].includes(role);
    if (!canSeeAccounting) return;

    // Inject the accounting section into the DOM if not already present
    if (!document.getElementById('accountingSection')) {
        _injectAccountingSection();
    }

    // Show accounting nav button
    const navBtn = document.getElementById('btnAccounting');
    if (navBtn) navBtn.style.display = '';

    // Load dashboard on first show
    _accLoadDashboard();
}
...
function showAccountingSection() {
    ...
    _setActiveNav('btnAccounting');
    _accLoadDashboard();
}

// ── DOM Injection ─────────────────────────────────────────────────────────────
function _injectAccountingSection() {
    const section = document.createElement('div');
    section.id    = 'accountingSection';
    section.style.display = 'none';
    section.innerHTML = `
    <div class="section-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:0.5rem;margin-bottom:1.25rem;">
      <h2 style="margin:0;font-size:1.35rem;font-weight:700;color:#e2e8f0;">
        🧾 Счетоводен портал
      </h2>
      <div style="display:flex;gap:0.5rem;flex-wrap:wrap;" id="accToolbar"></div>
    </div>

    <!-- Tabs -->
    <div style="display:flex;gap:0.25rem;margin-bottom:1.25rem;border-bottom:1px solid #1e293b;padding-bottom:0;">
      <button class="acc-tab-btn active" data-tab="dashboard"    onclick="_accSwitchTab('dashboard')">📊 Табло</button>
      <button class="acc-tab-btn"        data-tab="invoices"     onclick="_accSwitchTab('invoices')">🧾 Фактури</button>
      <button class="acc-tab-btn"        data-tab="handovers"    onclick="_accSwitchTab('handovers')">📂 Предавания</button>
      <button class="acc-tab-btn"        data-tab="audit"        onclick="_accSwitchTab('audit')">📋 Одит</button>
    </div>

    <!-- Tab panels -->
    <div id="accPanel-dashboard"></div>
    <div id="accPanel-invoices"  style="display:none"></div>
    <div id="accPanel-handovers" style="display:none"></div>
    <div id="accPanel-audit"     style="display:none"></div>

    <!-- Invoice Metadata Modal -->
    <div id="accInvoiceModal" style="display:none;position:fixed;inset:0;z-index:8000;
         background:rgba(0,0,0,0.7);backdrop-filter:blur(4px);align-items:center;justify-content:center;">
      <div style="background:#111e2c;border:1px solid #1e293b;border-radius:16px;
                  width:min(560px,95vw);max-height:90vh;overflow-y:auto;padding:2rem;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;">
          <h3 style="margin:0;color:#e2e8f0;font-size:1.1rem;">📝 Данни за фактура</h3>
          <button onclick="_accCloseInvoiceModal()" style="background:none;border:none;color:#64748b;font-size:1.4rem;cursor:pointer;">✕</button>
        </div>
        <div id="accInvoiceModalBody"></div>
      </div>
    </div>

    <!-- Pay Invoice Modal -->
    <div id="accPayModal" style="display:none;position:fixed;inset:0;z-index:8001;
         background:rgba(0,0,0,0.75);backdrop-filter:blur(4px);align-items:center;justify-content:center;">
      <div style="background:#111e2c;border:1px solid #1e293b;border-radius:16px;
                  width:min(480px,95vw);padding:2rem;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;">
          <h3 style="margin:0;color:#e2e8f0;font-size:1.1rem;">💳 Маркирай като платена</h3>
          <button onclick="_accClosePayModal()" style="background:none;border:none;color:#64748b;font-size:1.4rem;cursor:pointer;">✕</button>
        </div>
        <div id="accPayModalBody"></div>
      </div>
    </div>

    <!-- Handover Modal -->
    <div id="accHandoverModal" style="display:none;position:fixed;inset:0;z-index:8000;
         background:rgba(0,0,0,0.7);backdrop-filter:blur(4px);align-items:center;justify-content:center;">
      <div style="background:#111e2c;border:1px solid #1e293b;border-radius:16px;
                  width:min(640px,95vw);max-height:90vh;overflow-y:auto;padding:2rem;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;">
          <h3 style="margin:0;color:#e2e8f0;font-size:1.1rem;">📂 Ново счетоводно предаване</h3>
          <button onclick="_accCloseHandoverModal()" style="background:none;border:none;color:#64748b;font-size:1.4rem;cursor:pointer;">✕</button>
        </div>
        <div id="accHandoverModalBody"></div>
      </div>
    </div>`;

    // Tab styles
    if (!document.getElementById('acc-tab-style')) {
        const s = document.createElement('style');
        s.id = 'acc-tab-style';
        s.textContent = `
            .acc-tab-btn {
                background:none;border:none;border-bottom:2px solid transparent;
                color:#64748b;padding:0.6rem 1rem;font-size:0.85rem;font-weight:600;
                cursor:pointer;transition:all 0.15s;white-space:nowrap;
            }
            .acc-tab-btn:hover { color:#e2e8f0; }
            .acc-tab-btn.active { color:#e8682a;border-bottom-color:#e8682a; }
            .acc-kpi-card {
                background:#111e2c;border:1px solid #1e293b;border-radius:12px;
                padding:1.25rem 1.5rem;flex:1;min-width:140px;
            }
            .acc-kpi-card .kpi-val {
                font-size:2rem;font-weight:800;line-height:1;margin-bottom:0.25rem;
            }
            .acc-kpi-card .kpi-label {
                font-size:0.75rem;color:#64748b;text-transform:uppercase;
                letter-spacing:0.8px;font-weight:600;
            }
            .acc-table { width:100%;border-collapse:collapse; }
            .acc-table th {
                text-align:left;font-size:0.72rem;color:#64748b;text-transform:uppercase;
                letter-spacing:0.8px;font-weight:700;padding:0.6rem 1rem;
                border-bottom:1px solid #1e293b;background:#0b1520;
            }
            .acc-table td {
                padding:0.75rem 1rem;border-bottom:1px solid #1e2335;
                font-size:0.84rem;color:#cbd5e1;vertical-align:middle;
            }
            .acc-table tr:hover td { background:#172334; }
            .acc-status-pill {
            ...
            .acc-btn-primary { background:#e8682a;color:#fff; }
            .acc-btn-primary:hover { background:#f97316; }
            .acc-btn-secondary { background:#1e293b;color:#94a3b8;border:1px solid #334155; }
            .acc-btn-secondary:hover { background:#334155;color:#e2e8f0; }
            .acc-btn-success { background:#059669;color:#fff; }
            .acc-btn-success:hover { background:#10b981; }
            .acc-input {
                width:100%;padding:0.6rem 0.75rem;background:#0b1520;
                border:1px solid #1e293b;border-radius:8px;color:#e2e8f0;
                font-size:0.85rem;box-sizing:border-box;
            }
            .acc-input:focus { outline:none;border-color:#e8682a; }
            .acc-label { font-size:0.78rem;color:#94a3b8;margin-bottom:0.35rem;display:block; }
            .acc-form-row { display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1rem; }
            @media(max-width:560px){ .acc-form-row { grid-template-columns:1fr; } }
        `;
        document.head.appendChild(s);
    }

    document.querySelector('main') && document.querySelector('main').appendChild(section)
        || document.body.appendChild(section);
}

// ── Tab Switching ─────────────────────────────────────────────────────────────
function _accSwitchTab(tab) {
    _accCurrentTab = tab;
    document.querySelectorAll('.acc-tab-btn').forEach(b => {
        b.classList.toggle('active', b.dataset.tab === tab);
    });
    ['dashboard','invoices','handovers','audit'].forEach(t => {
        const el = document.getElementById(`accPanel-${t}`);
        if (el) el.style.display = t === tab ? '' : 'none';
    });
    if (tab === 'dashboard')  _accLoadDashboard();
    if (tab === 'invoices')   _accLoadInvoices();
    if (tab === 'handovers')  _accLoadHandovers();
    if (tab === 'audit')      _accLoadAudit();
}

// ── Dashboard Panel ───────────────────────────────────────────────────────────
async function _accLoadDashboard() {
    const panel = document.getElementById('accPanel-dashboard');
    if (!panel) return;
    panel.innerHTML = `<div style="color:#64748b;padding:2rem;text-align:center;">Зареждане...</div>`;

    try {
        const data = await apiGet('/accounting/dashboard');
        const c = data.invoiceCounts || {};
        const h = data.handoverCounts || {};

        const role = currentUser?.role;
        const canSend = ['admin','procurement','manager'].includes(role);

        panel.innerHTML = `
        <!-- Toolbar -->
        ${canSend ? `<div style="display:flex;gap:0.75rem;margin-bottom:1.5rem;flex-wrap:wrap;">
            <button class="acc-btn acc-btn-primary" onclick="_accOpenHandoverModal()">
                📂 Ново предаване към счетоводство
            </button>
            <button class="acc-btn acc-btn-secondary" onclick="_accSwitchTab('invoices')">
                🧾 Всички фактури
            </button>
        </div>` : ''}
        ...
        <div style="display:flex;gap:0.75rem;flex-wrap:wrap;margin-bottom:1.5rem;">
            <div class="acc-kpi-card" style="border-color:#ef4444;">
                <div class="kpi-val" style="color:#ef4444;">${c.overdue || 0}</div>
                <div class="kpi-label">Просрочени</div>
            </div>
            <div class="acc-kpi-card" style="border-color:#f59e0b;">
                <div class="kpi-val" style="color:#f59e0b;">${c.due_soon || 0}</div>
                <div class="kpi-label">До 7 дни</div>
            </div>
            <div class="acc-kpi-card">
                <div class="kpi-val" style="color:#94a3b8;">${c.unpaid || 0}</div>
                <div class="kpi-label">Неплатени</div>
            </div>
            <div class="acc-kpi-card" style="border-color:#059669;">
                <div class="kpi-val" style="color:#059669;">${c.paid_this_month || 0}</div>
                <div class="kpi-label">Платени (месец)</div>
            </div>
            <div class="acc-kpi-card">
                <div class="kpi-val" style="color:#e8682a;font-size:1.4rem;">
                    ${_fmtCurrency(c.total_outstanding || 0, 'EUR')}
                </div>
                <div class="kpi-label">Дължими общо</div>
            </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem;">
            ...
            <!-- Overdue -->
            <div style="background:#111e2c;border:1px solid #1e293b;border-radius:12px;padding:1.25rem;">
                <h4 style="margin:0 0 1rem;color:#ef4444;font-size:0.85rem;text-transform:uppercase;letter-spacing:0.8px;">
                    🔴 Просрочени фактури
                </h4>
                ${data.overdueList?.length ? data.overdueList.map(inv => `
                <div style="display:flex;justify-content:space-between;align-items:center;
                            padding:0.6rem 0;border-bottom:1px solid #1e2335;cursor:pointer;"
                     onclick="_accOpenPayModal(${inv.id})">
                    <div>
                        <div style="font-size:0.82rem;font-weight:600;color:#e2e8f0;">
                            ${escapeHtml(inv.invoice_number || 'Без номер')}
                        </div>
                        <div style="font-size:0.7rem;color:#ef4444;">
                            Просрочена с ${inv.days_overdue} ${inv.days_overdue===1?'ден':'дни'}
                        </div>
                    </div>
                    <div style="text-align:right;">
                        <div style="font-size:0.85rem;font-weight:700;color:#ef4444;">
                            ${_fmtCurrency(inv.amount_total, inv.currency)}
                        </div>
                    </div>
                </div>`).join('') : `<div style="color:#475569;font-size:0.82rem;">✅ Няма просрочени</div>`}
            </div>

            <!-- Due Soon -->
            <div style="background:#111e2c;border:1px solid #1e293b;border-radius:12px;padding:1.25rem;">
                <h4 style="margin:0 0 1rem;color:#f59e0b;font-size:0.85rem;text-transform:uppercase;letter-spacing:0.8px;">
                    ⏰ Предстоящи плащания (7 дни)
                </h4>
                ${data.dueSoonList?.length ? data.dueSoonList.map(inv => `
                <div style="display:flex;justify-content:space-between;align-items:center;
                            padding:0.6rem 0;border-bottom:1px solid #1e2335;cursor:pointer;"
                     onclick="_accOpenPayModal(${inv.id})">
                    <div>
                        ...
                        <div style="font-size:0.7rem;color:#f59e0b;">
                            ${inv.days_until_due === 0 ? 'Днес!' : `${inv.days_until_due} дни`} · ${_fmtDate(inv.due_date)}
                        </div>
                    </div>
                    <div style="text-align:right;">
                ...
                </div>`).join('') : `<div style="color:#475569;font-size:0.82rem;">✅ Няма предстоящи</div>`}
            </div>

        </div>

        <!-- Recent Handovers -->
        <div style="background:#111e2c;border:1px solid #1e293b;border-radius:12px;padding:1.25rem;margin-top:1.25rem;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;">
                <h4 style="margin:0;color:#94a3b8;font-size:0.85rem;text-transform:uppercase;letter-spacing:0.8px;">
                    📂 Последни предавания
                </h4>
                <button class="acc-btn acc-btn-secondary" onclick="_accSwitchTab('handovers')" style="font-size:0.72rem;">Виж всички</button>
            </div>
            ${data.recentHandovers?.length ? `
            <table class="acc-table">
                <thead><tr>
                    <th>#</th><th>Изпратено от</th><th>Дата</th><th>Документи</th><th>Статус</th><th></th>
                </tr></thead>
                <tbody>
                ${data.recentHandovers.map(h => `
                <tr>
                    <td>#${h.id}</td>
                    <td>${escapeHtml(h.sent_by_name || '—')}</td>
                    <td style="white-space:nowrap;">${_fmtDate(h.sent_at)}</td>
                    <td>${h.document_count}</td>
                    <td>${_accHandoverStatusPill(h.status)}</td>
                    <td>
                        <button class="acc-btn acc-btn-secondary" onclick="_accViewHandover(${h.id})"
                                style="font-size:0.72rem;padding:0.3rem 0.6rem;">Преглед</button>
                    </td>
                </tr>`).join('')}
                </tbody>
            </table>` : `<div style="color:#475569;font-size:0.82rem;">Няма предавания.</div>`}
        </div>`;

    } catch (err) {
        panel.innerHTML = `<div style="color:#ef4444;padding:1rem;">Грешка при зареждане: ${err.message}</div>`;
    }
}

// ── Invoices Panel ────────────────────────────────────────────────────────────
async function _accLoadInvoices(filter) {
    if (filter) _accInvoiceFilter = filter;
    const panel = document.getElementById('accPanel-invoices');
    if (!panel) return;
    panel.innerHTML = `<div style="color:#64748b;padding:2rem;text-align:center;">Зареждане...</div>`;

    try {
        const data = await apiGet(`/accounting/invoices?status=${_accInvoiceFilter}&limit=50`);
        const c    = data.counts || {};
        const role = currentUser?.role;
        const canPay  = ['accounting','admin'].includes(role);
        const canEdit = ['admin','procurement','manager'].includes(role);

        const filters = [
            { key: 'all',      label: `Всички` },
            { key: 'unpaid',   label: `Неплатени (${c.cnt_unpaid||0})` },
            { key: 'overdue',  label: `🔴 Просрочени (${c.cnt_overdue||0})` },
            { key: 'upcoming', label: `⏰ Скоро (${c.cnt_upcoming||0})` },
            { key: 'paid',     label: `✅ Платени (${c.cnt_paid||0})` },
        ];

        panel.innerHTML = `
        <div style="display:flex;gap:0.5rem;flex-wrap:wrap;margin-bottom:1rem;">
            ${filters.map(f => `
            <button class="acc-btn ${_accInvoiceFilter===f.key?'acc-btn-primary':'acc-btn-secondary'}"
                    onclick="_accLoadInvoices('${f.key}')" style="font-size:0.78rem;">
                ${f.label}
            </button>`).join('')}
        </div>

        ${data.invoices?.length ? `
        <div style="background:#111e2c;border:1px solid #1e293b;border-radius:12px;overflow:hidden;">
            <table class="acc-table">
                <thead><tr>
                    <th>Фактура №</th>
                    <th>Тип</th>
                ...
                ${data.invoices.map(inv => {
                    const isOverdue = inv.payment_status === 'unpaid' && inv.days_until_due < 0;
                    const isDueSoon = inv.payment_status === 'unpaid' && inv.days_until_due >= 0 && inv.days_until_due <= 3;
                    return `
                    <tr>
                        <td style="font-weight:700;color:${isOverdue?'#ef4444':isDueSoon?'#f59e0b':'#e2e8f0'};">
                            ${escapeHtml(inv.invoice_number || '—')}
                            ${isOverdue ? `<br><span style="font-size:0.68rem;color:#ef4444;">⚠ ${Math.abs(inv.days_until_due)} дни просрочена</span>` : ''}
                            ${isDueSoon && !isOverdue ? `<br><span style="font-size:0.68rem;color:#f59e0b;">⏰ ${inv.days_until_due} дни</span>` : ''}
                        </td>
                        <td style="font-size:0.78rem;">${inv.document_type === 'proforma_invoice' ? 'Проформа' : 'Фактура'}</td>
                        <td style="font-size:0.78rem;">${escapeHtml(inv.supplier_name || '—')}</td>
                        <td style="white-space:nowrap;font-size:0.78rem;">${_fmtDate(inv.invoice_date)}</td>
                        <td style="white-space:nowrap;font-size:0.78rem;color:${isOverdue?'#ef4444':isDueSoon?'#f59e0b':'inherit'};">
                            ${_fmtDate(inv.due_date)}
                        </td>
                        <td style="font-weight:700;white-space:nowrap;color:#e8682a;">
                            ${_fmtCurrency(inv.amount_total, inv.currency)}
                        </td>
                        <td>${_accPayStatusPill(inv.payment_status)}</td>
                        <td>
                            <div style="display:flex;gap:0.35rem;flex-wrap:nowrap;">
                                ${canEdit ? `<button class="acc-btn acc-btn-secondary" onclick="_accOpenInvoiceModal(${inv.document_id})"
                                        title="Редактирай данни" style="font-size:0.7rem;padding:0.25rem 0.5rem;">✏️</button>` : ''}
                                ${canPay && inv.payment_status === 'unpaid' ?
                                    `<button class="acc-btn acc-btn-success" onclick="_accOpenPayModal(${inv.id})"
                                             style="font-size:0.7rem;padding:0.25rem 0.6rem;">💳 Плати</button>` : ''}
                                <button class="acc-btn acc-btn-secondary" onclick="openDocumentAuthenticated(${inv.document_id})"
                                        title="Виж документ" style="font-size:0.7rem;padding:0.25rem 0.5rem;">👁</button>
                            </div>
                        </td>
                    </tr>`;
                }).join('')}
        ...
        </div>` : `<div style="color:#475569;padding:2rem;text-align:center;background:#111e2c;border-radius:12px;">
            Няма фактури за избрания филтър.
        </div>`}`;

    } catch (err) {
        panel.innerHTML = `<div style="color:#ef4444;padding:1rem;">Грешка: ${err.message}</div>`;
    }
}

// ── Handovers Panel ───────────────────────────────────────────────────────────
async function _accLoadHandovers() {
    const panel = document.getElementById('accPanel-handovers');
    if (!panel) return;
    panel.innerHTML = `<div style="color:#64748b;padding:2rem;text-align:center;">Зареждане...</div>`;

    try {
        const data  = await apiGet('/accounting/handovers?limit=30');
        const role  = currentUser?.role;
        const canSend = ['admin','procurement','manager'].includes(role);

        panel.innerHTML = `
        ${canSend ? `<div style="margin-bottom:1rem;">
            <button class="acc-btn acc-btn-primary" onclick="_accOpenHandoverModal()">
                📂 Ново предаване
            </button>
        </div>` : ''}
        ...
        <div style="background:#111e2c;border:1px solid #1e293b;border-radius:12px;overflow:hidden;">
            <table class="acc-table">
                <thead><tr>
                    <th>#</th><th>Изпратено от</th><th>Дата</th>
                    <th>Документи</th><th>Email</th><th>Статус</th><th></th>
                </tr></thead>
                <tbody>
                ${data.handovers.map(h => `
                <tr>
                    <td style="font-weight:700;">#${h.id}</td>
                    <td>${escapeHtml(h.sent_by_name || '—')}</td>
                    <td style="white-space:nowrap;font-size:0.78rem;">${_fmtDate(h.sent_at)}</td>
                    <td>${h.document_count}</td>
                    <td>${h.email_sent ? `<span style="color:#059669;">✓ Изпратен</span>` : `<span style="color:#64748b;">—</span>`}</td>
                    <td>${_accHandoverStatusPill(h.status)}</td>
                    <td>
                        <div style="display:flex;gap:0.35rem;">
                            <button class="acc-btn acc-btn-secondary" onclick="_accViewHandover(${h.id})"
                                    style="font-size:0.72rem;padding:0.3rem 0.6rem;">Преглед</button>
                            <button class="acc-btn acc-btn-secondary" onclick="_accDownloadZip(${h.id})"
                                    title="Изтегли ZIP" style="font-size:0.72rem;padding:0.3rem 0.6rem;">⬇ ZIP</button>
                        </div>
                    </td>
                </tr>`).join('')}
                </tbody>
...
// ── Audit Panel ───────────────────────────────────────────────────────────────
async function _accLoadAudit() {
    const panel = document.getElementById('accPanel-audit');
    if (!panel) return;
    panel.innerHTML = `<div style="color:#64748b;padding:2rem;text-align:center;">Зареждане...</div>`;

    try {
        const data = await apiGet('/accounting/audit-log?limit=50');

        const eventLabel = {
            handover_sent:         '📂 Предаване изпратено',
            handover_acknowledged: '✅ Предаване потвърдено',
            invoice_paid:          '💳 Фактура платена',
            invoice_meta_saved:    '✏️ Данни за фактура',
            reminder_sent:         '⏰ Напомняне изпратено',
        };
        ...
        <div style="background:#111e2c;border:1px solid #1e293b;border-radius:12px;overflow:hidden;">
            <table class="acc-table">
                <thead><tr>
                    <th>Събитие</th><th>Извършено от</th>
                ...
                <tr>
                    <td style="white-space:nowrap;font-size:0.78rem;">
                        ${eventLabel[l.event_type] || l.event_type}
                    </td>
                    <td style="font-size:0.78rem;">${escapeHtml(l.actor_name || l.actor_name_resolved || 'System')}</td>
                    <td style="font-size:0.78rem;color:#94a3b8;">${escapeHtml(l.description || '')}</td>
                    <td style="white-space:nowrap;font-size:0.75rem;color:#64748b;">${_fmtDateTime(l.created_at)}</td>
                </tr>`).join('') : `<tr><td colspan="4" style="text-align:center;color:#475569;">Няма записи</td></tr>`}
                </tbody>
            </table>
        </div>`;
...
// ── Invoice Metadata Modal ────────────────────────────────────────────────────
async function _accOpenInvoiceModal(documentId) {
    // Ensure accounting section is injected (may be called from order detail before portal is shown)
    if (!document.getElementById('accInvoiceModal')) {
        if (typeof _injectAccountingSection === 'function') _injectAccountingSection();
    }
    const modal = document.getElementById('accInvoiceModal');
    const body  = document.getElementById('accInvoiceModalBody');
    if (!modal || !body) { console.error('accInvoiceModal not found'); return; }
    modal.style.display = 'flex';
    body.innerHTML = `<div style="color:#64748b;text-align:center;padding:1rem;">Зареждане...</div>`;

    try {
        const data = await apiGet(`/accounting/invoice-meta/${documentId}`);
        const m                  = data.meta || {};
        const costCenters        = data.costCenters || [];
        const supplierSuggestion = data.supplierSuggestion || '';
        const linkedOrders       = data.linkedOrders || [];

        // Auto-fill supplier if not yet set
        const supplierVal = m.supplier_name || supplierSuggestion;

        // Cost center display
        const ccHtml = costCenters.length
            ? `<div style="background:#0b1520;border:1px solid #1e293b;border-radius:8px;padding:0.6rem 0.85rem;margin-bottom:1rem;">
                <div style="font-size:0.72rem;color:#64748b;text-transform:uppercase;letter-spacing:0.8px;margin-bottom:0.35rem;">Разходни центрове (от поръчките)</div>
                ${costCenters.map(cc => `<span style="display:inline-block;background:#172334;border:1px solid #2d3e6b;border-radius:6px;padding:2px 10px;font-size:0.78rem;color:#e2e8f0;margin:2px;font-weight:600;">${escapeHtml(cc.code)}${cc.name ? ` — ${escapeHtml(cc.name)}` : ''}</span>`).join('')}
               </div>`
            : '';

        const linkedOrdersHtml = linkedOrders.length
            ? `<div style="font-size:0.73rem;color:#64748b;margin-bottom:0.85rem;">
                🔗 Свързана с поръчки: ${linkedOrders.map(id => `<a href="#" onclick="return false;" style="color:#e8682a;">#${id}</a>`).join(', ')}
               </div>`
            : '';

        body.innerHTML = `
        <form id="accInvoiceForm">
            <input type="hidden" name="documentId" value="${documentId}">
            ${ccHtml}
            ${linkedOrdersHtml}
            <div class="acc-form-row">
                <div>
                    <label class="acc-label">Номер на фактура</label>
                    <input class="acc-input" name="invoice_number" value="${escapeHtml(m.invoice_number||'')}" placeholder="Напр. INV-2024-001">
                </div>
                <div>
                    <label class="acc-label">Тип</label>
                    <select class="acc-input" name="document_type" disabled style="opacity:0.6;">
                        <option>Фактура / Проформа</option>
                    </select>
                </div>
            </div>
            <div class="acc-form-row">
                <div>
                    <label class="acc-label">Дата на фактура</label>
                    <input class="acc-input" type="date" name="invoice_date" value="${m.invoice_date ? m.invoice_date.split('T')[0] : ''}">
                </div>
                <div>
                    <label class="acc-label">Краен срок за плащане *</label>
                    <input class="acc-input" type="date" name="due_date" value="${m.due_date ? m.due_date.split('T')[0] : ''}" required>
                </div>
            </div>
            <div class="acc-form-row">
                <div>
                    <label class="acc-label">Нетна сума</label>
                    <input class="acc-input" type="number" step="0.01" name="amount_net" value="${m.amount_net||''}" placeholder="0.00">
                </div>
                <div>
                    <label class="acc-label">ДДС</label>
                    <input class="acc-input" type="number" step="0.01" name="amount_vat" value="${m.amount_vat||''}" placeholder="0.00">
                </div>
            </div>
            <div class="acc-form-row">
                <div>
                    <label class="acc-label">Обща сума *</label>
                    <input class="acc-input" type="number" step="0.01" name="amount_total" value="${m.amount_total||''}" placeholder="0.00" required>
                </div>
                <div>
                    <label class="acc-label">Валута</label>
                    <select class="acc-input" name="currency">
                        ${['EUR','USD','GBP','CHF'].map(c => `<option ${(m.currency||'EUR')===c?'selected':''}>${c}</option>`).join('')}
                    </select>
                </div>
            </div>
            <div style="margin-bottom:1rem;">
                <label class="acc-label">Доставчик${supplierSuggestion && !m.supplier_name ? ' <span style="font-size:0.7rem;color:#e8682a;">(auto)</span>' : ''}</label>
                <input class="acc-input" name="supplier_name" value="${escapeHtml(supplierVal)}" placeholder="Наименование на доставчика">
            </div>
            <div class="acc-form-row">
                <div>
                    <label class="acc-label">Условия за плащане</label>
                    <select class="acc-input" name="payment_terms">
                        ${['','Net 7','Net 14','Net 30','Net 45','Net 60','Незабавно','При доставка'].map(pt =>
                            `<option ${(m.payment_terms||'')===pt?'selected':''}>${pt}</option>`
                        ).join('')}
                    </select>
                </div>
            </div>
            <div style="margin-bottom:1.5rem;">
                <label class="acc-label">Бележки</label>
                <textarea class="acc-input" name="notes" rows="2" style="resize:vertical;">${escapeHtml(m.notes||'')}</textarea>
            </div>
            <div style="display:flex;gap:0.75rem;justify-content:flex-end;">
                <button type="button" class="acc-btn acc-btn-secondary" onclick="_accCloseInvoiceModal()">Откажи</button>
                <button type="submit" class="acc-btn acc-btn-primary" id="accInvoiceSaveBtn">💾 Запази</button>
            </div>
        </form>`;

        // Auto-calculate total on net+vat change
        const netEl = body.querySelector('[name="amount_net"]');
        const vatEl = body.querySelector('[name="amount_vat"]');
        const totEl = body.querySelector('[name="amount_total"]');
        [netEl, vatEl].forEach(el => el.addEventListener('input', () => {
            const n = parseFloat(netEl.value) || 0;
            const v = parseFloat(vatEl.value) || 0;
            if (n || v) totEl.value = (n + v).toFixed(2);
        }));

        body.querySelector('#accInvoiceForm').addEventListener('submit', async (e) => {
            e.preventDefault();
            const btn = body.querySelector('#accInvoiceSaveBtn');
            btn.disabled = true; btn.textContent = 'Запазване...';
            const fd = new FormData(e.target);
            const payload = Object.fromEntries(fd.entries());
            try {
                await apiPost(`/accounting/invoice-meta/${documentId}`, payload);
                _accCloseInvoiceModal();
                showToast('Данните са запазени успешно', 'success');
                // Refresh current tab
                if (_accCurrentTab === 'invoices') _accLoadInvoices();
                if (_accCurrentTab === 'dashboard') _accLoadDashboard();
            } catch (err) {
        ...
        body.innerHTML = `<div style="color:#ef4444;">Грешка: ${err.message}</div>`;
    }
}
function _accCloseInvoiceModal() {
    document.getElementById('accInvoiceModal').style.display = 'none';
}

// ── Pay Invoice Modal ─────────────────────────────────────────────────────────
async function _accOpenPayModal(invoiceMetaId) {
    const modal = document.getElementById('accPayModal');
    const body  = document.getElementById('accPayModalBody');
    modal.style.display = 'flex';

    // Fetch invoice details
    try {
        const data = await apiGet(`/accounting/invoices?status=all&limit=200`);
        const inv  = data.invoices?.find(i => i.id === invoiceMetaId);

        body.innerHTML = `
        ${inv ? `
        <div style="background:#0b1520;border-radius:8px;padding:1rem;margin-bottom:1.25rem;">
            <div style="font-size:0.78rem;color:#64748b;margin-bottom:0.25rem;">Фактура</div>
            <div style="font-size:1.1rem;font-weight:700;color:#e2e8f0;">${escapeHtml(inv.invoice_number || 'Без номер')}</div>
            <div style="font-size:0.85rem;color:#e8682a;font-weight:700;margin-top:0.25rem;">
                ${_fmtCurrency(inv.amount_total, inv.currency)}
            </div>
            ${inv.due_date ? `<div style="font-size:0.75rem;color:#64748b;margin-top:0.2rem;">Краен срок: ${_fmtDate(inv.due_date)}</div>` : ''}
        </div>` : ''}

        <form id="accPayForm">
            <div style="margin-bottom:1rem;">
                <label class="acc-label">Платежно нареждане (PDF или снимка)</label>
                <input class="acc-input" type="file" name="payment_slip"
                       accept=".pdf,.jpg,.jpeg,.png"
                       style="padding:0.4rem;">
                <div style="font-size:0.72rem;color:#475569;margin-top:0.3rem;">
                    Задължително за проформа фактури. По желание за фактури.
                </div>
            </div>
            <div style="margin-bottom:1.5rem;">
                <label class="acc-label">Бележка (незадължително)</label>
                <textarea class="acc-input" name="notes" rows="2" placeholder="Напр. платено чрез банков превод..."></textarea>
            </div>
            <div style="display:flex;gap:0.75rem;justify-content:flex-end;">
                <button type="button" class="acc-btn acc-btn-secondary" onclick="_accClosePayModal()">Откажи</button>
                <button type="submit" class="acc-btn acc-btn-success" id="accPayBtn">💳 Маркирай като платена</button>
            </div>
        </form>`;

        body.querySelector('#accPayForm').addEventListener('submit', async (e) => {
            e.preventDefault();
            const btn = body.querySelector('#accPayBtn');
            btn.disabled = true; btn.textContent = 'Обработка...';

            const fd = new FormData(e.target);
            try {
                const resp = await fetch(`${API_BASE}/accounting/invoices/${invoiceMetaId}/pay`, {
                    method: 'POST',
                    headers: { 'Authorization': 'Bearer ' + authToken },
                    body: fd,
                });
                ...
                _accClosePayModal();
                showToast('Фактурата е маркирана като платена ✅', 'success');
                _accLoadDashboard();
                if (_accCurrentTab === 'invoices') _accLoadInvoices();
            } catch (err) {
                showToast('Грешка: ' + err.message, 'error');
                btn.disabled = false; btn.textContent = '💳 Маркирай като платена';
            }
        });
...
function _accClosePayModal() {
    document.getElementById('accPayModal').style.display = 'none';
}

// ── Handover Modal ────────────────────────────────────────────────────────────
async function _accOpenHandoverModal() {
    const modal = document.getElementById('accHandoverModal');
    const body  = document.getElementById('accHandoverModalBody');
    modal.style.display = 'flex';
    body.innerHTML = `<div style="color:#64748b;text-align:center;padding:1rem;">Зареждане на документи...</div>`;

    try {
        const all = await apiGet('/documents');

        // Only show accounting-relevant document types that have NOT been handed over
        const accTypes = ['invoice','proforma_invoice','delivery_note','purchase_order'];
        const docs = (all.documents || []).filter(d =>
            accTypes.includes(d.document_type) && d.status !== 'sent_to_accounting'
        );

        // Group by document type for clarity
        const typeOrder  = ['invoice','proforma_invoice','delivery_note','purchase_order'];
        const typeLabels = {
            invoice:          '🧾 Фактури',
            proforma_invoice: '📋 Проформа фактури',
            delivery_note:    '📦 Доставни бележки',
            purchase_order:   '🛒 Покупателни поръчки',
        };
        const groups = {};
        for (const t of typeOrder) groups[t] = docs.filter(d => d.document_type === t);
        const totalDocs = docs.length;

        // Fetch invoice metadata for all relevant docs in one call
        // (batch fetch via invoices list)
        const invData = await apiGet('/accounting/invoices?status=all&limit=500');
        const metaMap = {};
        for (const inv of (invData.invoices || [])) metaMap[inv.document_id] = inv;

        const renderGroup = (type, items) => {
            if (!items.length) return '';
            return `
            <tr class="acc-group-header">
                <td colspan="6" style="background:#0b1520;font-size:0.72rem;font-weight:700;
                    color:#e8682a;text-transform:uppercase;letter-spacing:0.8px;
                    padding:0.45rem 1rem;border-bottom:1px solid #1e293b;">
                    ${typeLabels[type]} (${items.length})
                </td>
            </tr>
            ${items.map(d => {
                const meta = metaMap[d.id];
                const invNum  = meta?.invoice_number  || '—';
                const dueDate = meta?.due_date         ? _fmtDate(meta.due_date) : '—';
                const amount  = meta?.amount_total     ? _fmtCurrency(meta.amount_total, meta.currency) : '—';
                const orderIds = (d.linked_order_ids || []).map(id => `#${id}`).join(', ') || '—';
                const hasMeta  = !!meta?.invoice_number;
                return `<tr>
                    <td style="width:36px;"><input type="checkbox" class="acc-doc-cb" value="${d.id}" checked></td>
                    <td style="white-space:nowrap;">
                        ${hasMeta
                            ? `<span style="color:#059669;font-size:0.75rem;font-weight:700;" title="Данните за фактурата са попълнени">✅</span>`
                            : `<span style="color:#f59e0b;font-size:0.75rem;" title="Липсват данни за фактурата — препоръчително е да ги попълните">⚠️</span>`
                        }
                        <span style="font-size:0.72rem;color:#94a3b8;margin-left:0.25rem;">${escapeHtml(invNum)}</span>
                    </td>
                    <td style="color:#e2e8f0;font-size:0.78rem;max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${escapeHtml(d.file_name)}">${escapeHtml(d.file_name)}</td>
                    <td style="white-space:nowrap;font-size:0.75rem;${meta?.due_date && new Date(meta.due_date)<new Date() ? 'color:#ef4444;font-weight:700;' : 'color:#94a3b8;'}">${dueDate}</td>
                    <td style="white-space:nowrap;font-size:0.75rem;color:#e8682a;">${amount}</td>
                    <td style="font-size:0.72rem;color:#64748b;">${orderIds}</td>
                </tr>`;
            }).join('')}`;
        };
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.75rem;">
            <p style="color:#94a3b8;font-size:0.83rem;margin:0;">
                Само <strong style="color:#e2e8f0;">непредадени</strong> фактури, проформи и доставни бележки.
                ⚠️ = липсват данни за фактурата.
            </p>
            <span style="background:#e8682a20;color:#e8682a;border:1px solid #e8682a40;border-radius:20px;padding:2px 10px;font-size:0.75rem;font-weight:700;white-space:nowrap;">
                ${totalDocs} документа
            </span>
        </div>

        ${totalDocs === 0
            ? `<div style="color:#475569;background:#0b1520;border-radius:8px;padding:1.5rem;margin-bottom:1rem;text-align:center;">
                ✅ Всички документи вече са предадени към счетоводството.
               </div>`
            : `<div style="max-height:320px;overflow-y:auto;margin-bottom:1rem;border:1px solid #1e293b;border-radius:8px;overflow:hidden;">
                <table class="acc-table" style="font-size:0.78rem;">
                    <thead><tr>
                        <th style="width:36px;">
                            <input type="checkbox" id="accSelectAll" checked
                                onchange="document.querySelectorAll('.acc-doc-cb').forEach(cb=>cb.checked=this.checked)">
                        </th>
                        <th>Фактура №</th>
                        <th>Файл</th>
                        <th>Краен срок</th>
                    ...
                    <tbody>
                        ${typeOrder.map(t => renderGroup(t, groups[t])).join('')}
                    </tbody>
                </table>
               </div>`
        }
        ...
        <div style="margin-bottom:1.25rem;">
            <label class="acc-label">Бележка към счетоводството (незадължително)</label>
            <textarea class="acc-input" id="accHandoverNotes" rows="2"
                      placeholder="Напр. фактури за месец април 2026..."></textarea>
        </div>
        <div style="display:flex;gap:0.75rem;justify-content:flex-end;">
            <button class="acc-btn acc-btn-secondary" onclick="_accCloseHandoverModal()">Откажи</button>
            <button class="acc-btn acc-btn-primary" id="accSendHandoverBtn"
                    onclick="_accSubmitHandover()" ${totalDocs===0?'disabled':''}>
                📂 Изпрати предаването
            </button>
        </div>`;
...
async function _accSubmitHandover() {
    const btn = document.getElementById('accSendHandoverBtn');
    btn.disabled = true; btn.textContent = 'Обработка...';

    const selected = Array.from(document.querySelectorAll('.acc-doc-cb:checked')).map(cb => parseInt(cb.value));
    const notes    = document.getElementById('accHandoverNotes')?.value || '';

    if (!selected.length) {
        showToast('Изберете поне един документ', 'error');
        btn.disabled = false; btn.textContent = '📂 Изпрати предаването';
        return;
    }

    try {
        const result = await apiPost('/accounting/handover', { documentIds: selected, notes });
        _accCloseHandoverModal();
        showToast(result.message || 'Предаването е изпратено успешно', 'success');
        _accLoadDashboard();
        _accLoadHandovers();
    } catch (err) {
        showToast('Грешка: ' + err.message, 'error');
...
function _accCloseHandoverModal() {
    document.getElementById('accHandoverModal').style.display = 'none';
}

// ── View Handover ─────────────────────────────────────────────────────────────
async function _accViewHandover(handoverId) {
    const modal = document.getElementById('accHandoverModal');
    const body  = document.getElementById('accHandoverModalBody');
    modal.style.display = 'flex';
    body.innerHTML = `<div style="color:#64748b;text-align:center;padding:1rem;">Зареждане...</div>`;
    ...
    try {
        const data = await apiGet(`/accounting/handovers/${handoverId}`);
        const h = data.handover;
        const docs = data.documents || [];
        const canAck = ['accounting','admin'].includes(currentUser?.role) && h.status === 'pending';

        body.innerHTML = `
        <div style="display:flex;gap:0.75rem;margin-bottom:1.25rem;flex-wrap:wrap;">
            <div style="background:#0b1520;border-radius:8px;padding:0.75rem 1rem;flex:1;min-width:120px;">
                <div style="font-size:0.7rem;color:#64748b;">Изпратено от</div>
                <div style="font-weight:700;color:#e2e8f0;">${escapeHtml(h.sent_by_name||'—')}</div>
            </div>
            <div style="background:#0b1520;border-radius:8px;padding:0.75rem 1rem;flex:1;min-width:120px;">
                <div style="font-size:0.7rem;color:#64748b;">Дата</div>
                <div style="font-weight:700;color:#e2e8f0;">${_fmtDate(h.sent_at)}</div>
            </div>
            <div style="background:#0b1520;border-radius:8px;padding:0.75rem 1rem;flex:1;min-width:120px;">
                <div style="font-size:0.7rem;color:#64748b;">Статус</div>
                <div>${_accHandoverStatusPill(h.status)}</div>
            </div>
        </div>
        ${h.notes ? `<div style="background:#0b1520;border-radius:8px;padding:0.75rem 1rem;margin-bottom:1rem;
                                  font-size:0.82rem;color:#94a3b8;">${escapeHtml(h.notes)}</div>` : ''}

        <table class="acc-table" style="margin-bottom:1.25rem;">
            <thead><tr><th>Тип</th><th>Файл</th><th>Фактура №</th><th>Срок</th><th>Статус</th><th></th></tr></thead>
            <tbody>
            ${docs.map(d => `
            <tr>
                <td style="font-size:0.75rem;">${d.document_type}</td>
                <td style="font-size:0.78rem;color:#e2e8f0;">${escapeHtml(d.file_name)}</td>
                <td style="font-size:0.75rem;">${escapeHtml(d.invoice_number||'—')}</td>
                <td style="font-size:0.75rem;white-space:nowrap;">${_fmtDate(d.due_date)}</td>
                <td>${d.invoice_meta_id ? _accPayStatusPill(d.payment_status||'unpaid') : '<span style="color:#475569;font-size:0.72rem;">—</span>'}</td>
                <td>
                    <button class="acc-btn acc-btn-secondary" onclick="openDocumentAuthenticated(${d.id})"
                            style="font-size:0.7rem;padding:0.25rem 0.5rem;">👁</button>
                </td>
            </tr>`).join('')}
            </tbody>
        </table>
        ...
        <div style="display:flex;gap:0.75rem;justify-content:flex-end;flex-wrap:wrap;">
            <button class="acc-btn acc-btn-secondary" onclick="_accDownloadZip(${handoverId})">
                ⬇ Изтегли ZIP
            </button>
            ${canAck ? `<button class="acc-btn acc-btn-success" onclick="_accAcknowledgeHandover(${handoverId})">
                ✅ Потвърди получаването
            </button>` : ''}
            <button class="acc-btn acc-btn-secondary" onclick="_accCloseHandoverModal()">Затвори</button>
        </div>`;
...
async function _accAcknowledgeHandover(handoverId) {
    try {
        await apiPost(`/accounting/handovers/${handoverId}/acknowledge`, {});
        showToast('Предаването е потвърдено', 'success');
        _accCloseHandoverModal();
        _accLoadDashboard();
        _accLoadHandovers();
    } catch (err) {
...
function _accDownloadZip(handoverId) {
    const a = document.createElement('a');
    a.href = `${API_BASE}/accounting/handovers/${handoverId}/download-zip`;
    // Fetch with auth
    fetch(a.href, { headers: { 'Authorization': 'Bearer ' + authToken } })
        .then(r => r.blob())
        .then(blob => {
            const url = URL.createObjectURL(blob);
            a.href = url;
            a.download = `handover-${handoverId}.zip`;
            a.click();
            setTimeout(() => URL.revokeObjectURL(url), 5000);
        })
        .catch(() => showToast('Грешка при изтегляне', 'error'));
}
...
function _fmtDate(d) {
    if (!d) return '—';
    const dt = new Date(d);
    return isNaN(dt) ? d : dt.toLocaleDateString('bg-BG', { day:'2-digit', month:'2-digit', year:'numeric' });
}
function _fmtDateTime(d) {
    if (!d) return '—';
    const dt = new Date(d);
    return isNaN(dt) ? d : dt.toLocaleString('bg-BG', { day:'2-digit', month:'2-digit', year:'numeric', hour:'2-digit', minute:'2-digit' });
}
function _fmtCurrency(amount, currency) {
    if (!amount) return '—';
    return parseFloat(amount).toLocaleString('bg-BG', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
        + ' ' + (currency || 'EUR');
}
function _accPayStatusPill(status) {
    const map = {
        unpaid:   ['#f59e0b','#78350f','⏳ Неплатена'],
        paid:     ['#059669','#052e16','✅ Платена'],
        overdue:  ['#ef4444','#7f1d1d','🔴 Просрочена'],
        disputed: ['#8b5cf6','#2e1065','⚠️ Оспорена'],
        cancelled:['#64748b','#1e293b','❌ Анулирана'],
    };
    const [bg,textColor,label] = map[status] || map.unpaid;
    return `<span class="acc-status-pill" style="background:${bg}20;color:${bg};border:1px solid ${bg}50;">${label}</span>`;
}
function _accHandoverStatusPill(status) {
    const map = {
        pending:      ['#f59e0b','⏳ Изчакващо'],
        acknowledged: ['#059669','✅ Потвърдено'],
        completed:    ['#3b82f6','🏁 Завършено'],
    };
    const [color,label] = map[status] || ['#64748b','—'];
    return `<span class="acc-status-pill" style="background:${color}20;color:${color};border:1px solid ${color}50;">${label}</span>`;
}

// ── Send Proforma for Payment ────────────────────────────────────────────────
async function _accSendProformaModal(documentId, fileName) {
    ...
    try {
        // Fetch accounting users + invoice meta
        const [usersRes, metaRes] = await Promise.all([
            apiGet('/accounting/users'),
            apiGet(`/accounting/invoice-meta/${documentId}`)
        ]);
        const users = usersRes.users || [];
        const meta  = metaRes.meta  || {};

        const metaInfoHtml = (meta.invoice_number || meta.amount_total)
            ? `<div style="background:#0b1520;border:1px solid #1e293b;border-radius:8px;
                           padding:0.65rem 0.9rem;margin-bottom:1rem;font-size:0.82rem;color:#94a3b8;">
                ${meta.invoice_number ? `<span style="color:#e2e8f0;font-weight:600;">№ ${escapeHtml(meta.invoice_number)}</span>&nbsp;&nbsp;` : ''}
                ${meta.amount_total   ? `<span style="color:#e8682a;font-weight:700;">${_fmtCurrency(meta.amount_total, meta.currency)}</span>&nbsp;&nbsp;` : ''}
                ${meta.due_date       ? `<span>Срок: <strong style="color:${new Date(meta.due_date)<new Date()?'#ef4444':'#e2e8f0'};">${_fmtDate(meta.due_date)}</strong></span>` : ''}
               </div>`
            : `<div style="background:#78350f20;border:1px solid #f59e0b40;border-radius:8px;
                           padding:0.65rem 0.9rem;margin-bottom:1rem;font-size:0.8rem;color:#f59e0b;">
                ⚠️ Препоръчително е да попълните данните за проформата (✏️ Фактура) преди изпращане.
               </div>`;

        const recipientHtml = users.length
            ? `<div class="acc-form-row" style="grid-template-columns:1fr;margin-bottom:0.85rem;">
                <div>
                    <label class="acc-label">Изпрати до счетоводител</label>
                    <select class="acc-input" id="accProformaRecipient">
                        <option value="">— Всички счетоводители —</option>
                        ${users.map(u => `<option value="${u.id}">${escapeHtml(u.name || u.username)} (${escapeHtml(u.email)})</option>`).join('')}
                    </select>
                </div>
               </div>`
            : `<div style="color:#ef4444;font-size:0.82rem;margin-bottom:0.85rem;">⚠️ Няма активни счетоводители в системата.</div>`;

        body.innerHTML = `
        <div style="margin-bottom:0.85rem;">
            <div style="font-size:0.72rem;color:#64748b;text-transform:uppercase;letter-spacing:0.8px;margin-bottom:0.3rem;">Проформа фактура</div>
            <div style="color:#e2e8f0;font-weight:600;font-size:0.9rem;">📋 ${escapeHtml(fileName)}</div>
        </div>
        ${metaInfoHtml}
        ${recipientHtml}
        <div style="margin-bottom:1rem;">
            <label class="acc-label">Бележка (незадължително)</label>
            <textarea class="acc-input" id="accProformaNotes" rows="2"
                      placeholder="Напр. моля платете до края на месеца..."></textarea>
        </div>
        <div style="display:flex;gap:0.75rem;justify-content:flex-end;">
            <button class="acc-btn acc-btn-secondary" onclick="_accCloseHandoverModal()">Откажи</button>
            <button class="acc-btn acc-btn-primary" id="accProformaSendBtn"
                    onclick="_accSubmitProformaPayment(${documentId})" ${!users.length?'disabled':''}>
                📤 Изпрати за плащане
            </button>
        </div>`;
...
async function _accSubmitProformaPayment(documentId) {
    const btn         = document.getElementById('accProformaSendBtn');
    const recipientId = document.getElementById('accProformaRecipient')?.value || '';
    const notes       = document.getElementById('accProformaNotes')?.value     || '';

    btn.disabled = true; btn.textContent = 'Изпращане...';
    try {
        const result = await apiPost('/accounting/handover', {
            documentIds: [documentId],
            notes,
            recipientId: recipientId ? parseInt(recipientId) : null
        });
        _accCloseHandoverModal();
        showToast(result.message || 'Проформата е изпратена за плащане', 'success');
        // Refresh doc list
        if (typeof loadOrderDocuments === 'function' && window._currentDetailOrderId) {
            loadOrderDocuments(window._currentDetailOrderId);
        }
    } catch (err) {
        showToast('Грешка: ' + err.message, 'error');
        btn.disabled = false; btn.textContent = '📤 Изпрати за плащане';
    }
}

// ── Expose globally ───────────────────────────────────────────────────────────
window.initAccountingPortal       = initAccountingPortal;
window.showAccountingSection      = showAccountingSection;
window._accSwitchTab              = _accSwitchTab;
window._accLoadInvoices           = _accLoadInvoices;
window._accOpenInvoiceModal       = _accOpenInvoiceModal;
window._accCloseInvoiceModal      = _accCloseInvoiceModal;
window._accOpenPayModal           = _accOpenPayModal;
window._accClosePayModal          = _accClosePayModal;
window._accOpenHandoverModal      = _accOpenHandoverModal;
window._accCloseHandoverModal     = _accCloseHandoverModal;
window._accSubmitHandover         = _accSubmitHandover;
window._accViewHandover           = _accViewHandover;
window._accAcknowledgeHandover    = _accAcknowledgeHandover;
window._accDownloadZip            = _accDownloadZip;
window._accSendProformaModal      = _accSendProformaModal;
window._accSubmitProformaPayment  = _accSubmitProformaPayment;
